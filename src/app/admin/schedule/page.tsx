...
import { Calendar, Clock, Video, Loader2, Check, XCircle, Search, Filter, X } from 'lucide-react';
...
export default function AdminSchedulePage() {
    ...
    const [selectedStatus, setSelectedStatus] = useState<MeetingStatus>('pending');
    ...
    const fetchSchedule = async () => {
        try {
            setIsLoading(true);
            setError(null);
            const response = await scheduleService.getAdminSchedule(new Date());
            setMeetings(response.schedules);
        } catch (err) {
    ...
    const filteredMeetings = useMemo(() => {
        return meetings.filter((meeting) => {
            ...
            const matchesSearch =
                ...
                meeting.student_name?.toLowerCase().includes(searchLower) ||
    ...
    const handleUpdateStatus = async () => {
        ...
        try {
            ...
            await scheduleService.updateMeetingStatus(selectedMeeting._id, {
    ...
    return (
        <div className="container mx-auto p-4 space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    <h1 className="text-3xl font-bold text-sky-900">Schedule Management</h1>
                    <p className="text-sky-700 mt-1">Manage and approve meeting requests</p>
                </div>
            </div>
                                    ...
                                    filteredMeetings.map((meeting) => (
                                        <TableRow key={meeting._id}>
                                            <TableCell className="font-medium">
                                                {meeting.student.full_name || 'Unknown Student'}
                                            </TableCell>
                                            <TableCell>
            ...
            <Dialog open={isStatusDialogOpen} onOpenChange={setIsStatusDialogOpen}>
                <DialogContent>
                    <DialogHeader>
                        <DialogTitle>
                            {selectedStatus === 'approved' ? 'Approve Meeting' : 'Reject Meeting'}
                        </DialogTitle>
                    </DialogHeader>
                    <div className="space-y-4">
